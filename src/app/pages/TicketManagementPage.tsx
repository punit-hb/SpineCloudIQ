...
import { AlertCircle, ArrowUpDown, BarChart3, Building2, Calendar, Check, CheckCircle, Columns3, Download, Edit, Filter, Mail, MoreVertical, Plus, Printer, RefreshCw, Search, Send, Upload, User, X } from "lucide-react";
import { CardFooter, CardHeader, CardMetaRow, CardStatusPill, EnterpriseAvatar, EntityCard, ViewModeSwitcher } from "../components/hb/listing";
...
function TicketMark({ priority }: { priority: Ticket["priority"] }) {
  return <EnterpriseAvatar icon={Mail} className={priority === "Critical" ? "text-red-600" : "text-primary"} />;
}

function DotBadge({ value, kind }: { value: string; kind: "status" | "priority" }) {
  const tone = kind === "priority"
    ? value === "Critical" ? "red" : value === "High" ? "amber" : value === "Medium" ? "blue" : "gray"
    : value === "Open" ? "blue" : value === "In Progress" ? "amber" : value === "Resolved" ? "green" : "gray";
...
{viewMode === "grid" ? <><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{currentData.map((ticket) => <EntityCard key={ticket.id} selected={selectedRowIds.includes(ticket.id)} onClick={() => handleViewTicket(ticket)}><CardHeader avatar={<TicketMark priority={ticket.priority} />} title={ticket.subject} subtitle={ticket.id} selected={selectedRowIds.includes(ticket.id)} onSelect={(checked) => toggleSelected(ticket.id, checked)} actions={renderActions(ticket)} /><div className="flex-1 space-y-2 text-sm"><CardMetaRow icon={Building2}>{ticket.clinicName}</CardMetaRow><CardMetaRow icon={User}>{ticket.clinicAdmin}</CardMetaRow><CardMetaRow icon={Mail}>{ticket.clinicAdminEmail}</CardMetaRow><CardMetaRow icon={Calendar}>{formatDate(ticket.lastUpdated)}</CardMetaRow></div><CardFooter className="justify-between"><DotBadge value={ticket.priority} kind="priority" /><DotBadge value={ticket.status} kind="status" /></CardFooter></EntityCard>)}</div>{renderPagination()}</> : null}
{viewMode === "list" ? <div className="overflow-hidden rounded-lg border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-950"><div className="divide-y divide-neutral-100 dark:divide-neutral-800">{currentData.map((ticket) => <div key={ticket.id} className={`flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between ${selectedRowIds.includes(ticket.id) ? "bg-primary/5" : ""}`}><div className="flex min-w-0 items-center gap-3"><input type="checkbox" checked={selectedRowIds.includes(ticket.id)} onChange={(e) => toggleSelected(ticket.id, e.target.checked)} className="h-4 w-4 rounded border-neutral-300 accent-primary" /><TicketMark priority={ticket.priority} /><div className="min-w-0"><button type="button" onClick={() => handleViewTicket(ticket)} className="truncate text-left text-sm font-semibold text-primary hover:underline">{ticket.subject}</button><p className="truncate text-xs text-neutral-500">{ticket.clinicName} • {ticket.id}</p></div></div><div className="flex flex-wrap items-center gap-2"><DotBadge value={ticket.priority} kind="priority" /><DotBadge value={ticket.status} kind="status" /><span className="text-xs text-neutral-500">{formatDate(ticket.lastUpdated)}</span>{renderActions(ticket)}</div></div>)}</div>{renderPagination()}</div> : null}
